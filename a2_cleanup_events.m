...
gridsize = gridsize*3;
xnode = [lalim(1),mean(lalim),lalim(2)];%[latitude_min, latitude_med, latitude_max]
...
for ie = 1:length(matfiles)
	...
	win_start(ie) = evotimes(ie) + min(dist(:)/5);%velocity5 for what?
...
for ie = 1:length(evlas)
	for je = 1:length(evlas)
        ...
        %label event that are close and has time overlap as not good
